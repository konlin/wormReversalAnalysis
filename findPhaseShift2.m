...
for k=1:length(psWormLength)
    totalPS=round(psWormLength(k))+totalPS;
end
curvatureMatrix=NaN(100+totalPS, length(wormReverse.WormVid)); %in percent worm body
for i=1:length(wormReverse.WormVid)
    for j=1:100+totalPS
        if(j <= 100)
            curvatureMatrix(currentShift+j,i)=curvdata(j,i);
        end
    end
    if(i ~= length(wormReverse.WormVid))
        currentShift=currentShift+round(psWormLength(i));
    end
end
constructedWave = zeros(100+totalPS,1);
for i=1:100+totalPS
    count=0;
    for j=1:length(wormReverse.WormVid)
